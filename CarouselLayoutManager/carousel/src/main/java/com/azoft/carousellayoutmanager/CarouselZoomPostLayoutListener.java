...
public class CarouselZoomPostLayoutListener implements CarouselLayoutManager.PostLayoutListener {
    ...
    public ItemTransformation transformChild(@NonNull final View child, final float itemPositionToCenterDiff, final int orientation) {
        ...
        //相当于就是说 Math.abs(itemPositionT oCenterDiff)值越大(离center就越远) scale系数就该越小
        ...
        //因此是要 itemPositionToCenterDiff 越大 scale值越小满足
        double v = 1 - 2 * StrictMath.atan(Math.abs(itemPositionToCenterDiff) + 1.0) / Math.PI;
        double v1 =2 * StrictMath.atan(Math.abs(itemPositionToCenterDiff)) / Math.PI;//[0,1)
